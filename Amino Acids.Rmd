...
fpath <- function(AA){
  ...
  if(AA == "D"|"Asp"|"d"|"asp"){
    path(1,10,13,14,13,15) + path(11,10,12)
  }
  
  if(AA == "E"|"Glu"|"e"|"glu"){
    path(1,10,13,16,17,16,18) + path(11,10,12) + path(14,13,15)
  }
  
  if(AA == "F"|"Phe"|"f"|"phe"){
    path(1,10,13,14,16,18,20,22,13,22,23) + path(11,10,12) 
    + path(15,14,16,17) + path(19,18,20,21)
  }
...
gAA <- graph.empty(directed=FALSE) + vertices(AABackbone,AAcid)
AABbPath <- path(1,2,3,2,4) + path(9,1,5,6) + path(7,5,8)
gAA <- gAA + AABbPath + fpath(AA)
